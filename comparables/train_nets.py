...
parser.add_argument('--Network',       type=str,   default='Guided_DRCNN', help='from {HyperPNN, Guided_DRCNN , GSA_SSR, DRCNN, Guided_DRCNN_ori, SSR}')
parser.add_argument('--Dataset',       type=str,   default='GF5_real',  help='training dataset from {CAVE, Chikusei, Harvard, Chikusei_v2, GF, CAVE_ori, GF_simu  }')
...
########## GF5_real #################  
    ...
    path      = 'E:\\0 HyperModel\\1_dealed_data\\GF5_real\\version_1\\'  
    train_hsi = np.load(path+'GF_data_real_train_cropH.npy')
    train_msi = np.load(path+'GF_data_real_train_cropM.npy')
    train_pan = np.load(path+'GF_data_real_train_cropP.npy')
    train_ref = np.load(path+'GF_data_real_train_cropR.npy') 
else:
    raise NotImplementedError("Dataset [%s] is not recognized." % args.Dataset)
 
Name      = "Net_%s_Data_%s_Epoch_%d_V1" %(args.Network, args.Dataset, args.end_epoch)  #  
...
# from models.DRCNN   import DRCNN, Guided_DRCNN_ori
