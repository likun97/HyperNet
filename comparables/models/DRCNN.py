...
# Created by: Kun Li
# "Deep Residual Learning for Boosting the Accuracy of Hyperspectral Pansharpening"  DOI: 10.1109/LGRS.2019.2945424
...
# refer to https://github.com/yxzheng24/IEEE_GRSL_DRCNN (TF version )  
#          https://github.com/jianzhangcs/ISTA-Net-PyTorch/blob/master/Train_CS_ISTA_Net.py


class DRCNN(nn.Module):    
    # def __init__(self, in_channels, mid_channels, out_channels, factor): 
...
# refer to https://github.com/yxzheng24/IEEE_GRSL_DRCNN (TF version )  /// Guided filter  https://github.com/haochange/imguidedfilter-opencv
import cv2 
import numpy as np
import sys
sys.path.append("..")
from utils.imguidedfilter import imguidedfilter
...
class Guided_DRCNN(nn.Module):     
    ...
    def forward(self, batch_h, batch_m, batch_p, batch_r):  
        ...
        # 2-Generating the Initialized HSI via Guided Filter   --- I_gf(:,:,j) = imguidedfilter(I_HS2(:,:,j),D_PAN,'NeighborhoodSize',15,'DegreeOfSmoothing',0.001^2);     
        # 3-H_res
        # =====================================================================
        ...
        for i in range(batch_p.shape[0]):
            # batch_p[i,:,:,0] = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply((batch_p[i,:,:,0]*255).astype('uint8'))
            # batch_p[i,:,:,0] = ((batch_p[i,:,:,0] - np.min(batch_p[i,:,:,0])) / (np.amax(batch_p[i,:,:,0]) - np.amin(batch_p[i,:,:,0])))
        ...
        for i in range(batch_h_up.shape[0]):
            for j in range(batch_h_up.shape[3]):
                # batch_Hini[i,:,:,j] = imguidedfilter(batch_h_up[i,:,:,j], batch_p[i,:,:,:], 'NeighborhoodSize',15,'DegreeOfSmoothing',0.001^2)
...
class Guided_DRCNN_ori(nn.Module):     
        
    def __init__(self, in_channels, mid_channels, out_channels, hp_ratio, n_select_bands, hm_ratio, n_bands):
        super(Guided_DRCNN_ori, self).__init__()
        self.in_channels    = in_channels
        self.mid_channels   = mid_channels
        self.out_channels   = out_channels
        self.hp_ratio       = hp_ratio
    ...
    def forward(self, batch_hini):
        H_block1 = self.conv1(batch_hini)
        H_block2 = self.conv2(H_block1)
        H_block2 = self.conv3(H_block2)
        H_block2 = self.conv4(H_block2)
        H_block2 = self.conv5(H_block2)
