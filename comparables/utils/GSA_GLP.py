...
def upsample_interp23(image, ratio):
    ...
    first = 1
    for z in range(1,np.int(np.log2(ratio))+1):                              ### M:P=1:3  45只能到90
    # for z in range(1,np.int(np.round(np.log2(ratio)))+1):
